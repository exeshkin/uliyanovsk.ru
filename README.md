...
Статический веб-сайт с элементами динамического контента через JSON. Сайт предоставляет информацию о церкви, включая:
...
Сайт использует **SSI (Server Side Includes)** для подключения header/footer через директивы nginx. На production-сервере SSI обрабатывается nginx, локально можно использовать любой сервер с поддержкой SSI.

**Локальная разработка:**
```bash
# Python (с поддержкой SSI через расширение или nginx)
cd uliyanovsk.ru
...
> **Примечание:** Для полной работы SSI-включений (header/footer) необходим nginx или аналогичный сервер с поддержкой SSI. Без SSI страницы будут отображаться без шапки и подвала.
...
├── minister-*.html         # Страницы служителей (6 шт.)
...
├── img/                    # Изображения
├── fonts/                  # Шрифты (Exo2)
├── files/                  # Медиафайлы (фото, видео, постеры)
...
| Технология | Назначение |
|------------|------------|
| **NGINX** | Веб-сервер + RTMP модуль + SSI |
...
- **SSI:** Подключение header.html/footer.html
- **Кэширование:**
...
- **services.json** — проповеди служителей (Анатолий Склонный, Илья Акатов, Виталий Сторожок, Владимир Рябов, Вячеслав Коваленко, Дмитрий Свистун)
- **ministers.json** — список служителей с путями к медиа
- **gallery.json** — фотоальбомы и видео-обзоры
- **psalms.json** — псалмы и песни по годам
...
Header и footer подключаются через SSI (обрабатывается nginx на production):
```html
...
3. **Псалмы:** Обновить `psalms.json`, загрузить MP4
4. **Уведомления:** Изменить `notify.json` (`on: "true"`, дата, изображение)
5. **Галерея:** Обновить `gallery.json`, загрузить фото
